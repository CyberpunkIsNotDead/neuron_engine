...
class Replies(models.Model):
    parent_thread = models.ForeignKey(
        ...
        blank=True,
        null=True,
        related_name='replies_to_thread'
        )
    ...
    child_thread = models.ForeignKey(
        OriginalPost,
        on_delete=models.CASCADE,
        blank=True,
        null=True
...
def uploads_path(instance, filename):
    return '{0}/uploads/{1}'.format(
        instance.board.id,
        filename
        )

def thumbnails_path(instance, filename):
    return '{0}/uploads/thumbnails/{1}'.format(
        instance.board.id,
        filename
        )
...
class Upload(models.Model):
    ...
    upload = models.FileField(
        upload_to=uploads_path,
        )
    thumbnail = models.FileField(
        upload_to=thumbnails_path,
        blank=True,
        null=True
        )
    def __str__(self):
